# AGENTS.md — SUTRANETRA

This file is the standing operating contract for any agent (Cursor, Claude Code, or otherwise) working in this repository. It does not change per prompt. Read it once per session before touching code, and re-read it if a session gets long enough that context has been compacted.
...
`CLAUDE.md` is the same contract for Claude Code; keep both files aligned if either changes.
...
A submission that only does correlation is not this project — it's the generic version this spec was written to beat. Attribution without the third layer leaves a score without a case file. If you are ever unsure which layer a piece of work serves, ask before writing it.
...
```
sutranetra/                 # product / shipped project name: SUTRANETRA
...
└── docs/
    ...
    ├── ethics.md
    ├── build-log.md      # per-prompt progress, metrics, and issue notes (required)
    └── VALIDATION-AND-NOVELTY.md
```
...
7. **Update documentation before closing the prompt.** Append a clear entry to `docs/build-log.md` (create the file if missing) covering what was built, which DoD items and tests passed, actual metrics produced, config profile used (`dev` / `demo`), and any issues, workarounds, or open questions. Issues and near-misses must be written plainly enough that the team can reuse them when judges ask questions or in interviews — do not leave failures only in chat history.
8. End your turn with a short status block: which Definition-of-Done items passed, which tests passed, and — if relevant — the actual metric produced (row counts, PR-AUC, number of findings, etc.), not just "done." Confirm that `docs/build-log.md` was updated.
...
## 5. Per-prompt documentation (required)

Clear documentation is part of every prompt's Definition of Done, not an optional follow-up.

- After each prompt finishes (pass or fail-stop), update `docs/build-log.md` with a dated section for that prompt (`NN-name`).
- Record: intent of the step, what changed (files/modules), commands run, DoD checklist results, test results, and honest metrics.
- Record issues explicitly: errors hit, false starts, degraded-path behavior, heuristic vs learned confidence, cache vs live network, and anything that would surprise a judge if discovered only under questioning.
- Prefer short, factual notes over narrative. The audience is future-you under interview pressure, not a blog reader.
- If `SPEC.md` or public-facing docs (`docs/ethics.md`, architecture notes) need to change because the implementation settled a detail, update those in the same pass — do not let docs drift behind the code.
- Chat summaries are not a substitute for `docs/build-log.md`.

## 6. Correctness rules that protect the pitch
...
## 8. Legal and ethical operating constraints — hard rules, not framing copy
...
Never say a prompt is "complete" or "working" without having just run the thing that proves it. If you have not executed the Definition of Done for the current prompt in this session, you do not yet know whether it's done, and you should say that instead of guessing. A prompt is also not complete until its `docs/build-log.md` entry exists and records both successes and issues.
